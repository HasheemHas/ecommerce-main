...
subtot = parseFloat(price.value) * parseFloat(qty.value);
 ...
 // alert(subtot.toFixed(2))
  ...
  //   });
    ...
    $.ajax({    //create an ajax request to load_page.php
        type:"POST",
        url: "../backend/cart/controller.php?action=edit&QTY"+productid+"="+ qty.value + "&TOT"+productid+"=" + subtot.toFixed(2),             
        dataType: "text",   //expect html to be returned  
...
subtot = parseFloat(price.value) * parseFloat(qty.value);
  ...
  $.ajax({    //create an ajax request to load_page.php
    ...
    var sum = 0;
    for(var i=0; i<items.length; i++) {
        var val = items[i].value ? items[i].value.replace(/,/g, '') : items[i].innerHTML.replace(/,/g, '');
...
// VALIDATE CUSTOMER
 ...
 // Validates Personal Info
   $(document).on("click", ".submit", function () { 
      ...
      // $.ajax({    //create an ajax request to load_page.php
        //     type:"POST",
        //     url: "../backend/customer/controller.php?action=add",             
        //     dataType: "text",   //expect html to be returned  
        //     data:{
...
// end viewing.................................
 ...
 $(document).on("click", ".orderid", function () {
   ...
   var action = $(this).data('action');
      ...
      $.ajax({    //create an ajax request to load_page.php
        type:"POST",
        url: "../backend/customer/listorderedproduct.php",             
        dataType: "text",   //expect html to be returned  
        ...
        success: function(data){                    
            ...
            if (action === 'cancel') {
                setTimeout(function() {
                    if (typeof toggleCancelForm === "function") toggleCancelForm();
                }, 50);
            } else if (action === 'return') {
                setTimeout(function() {
                    if (typeof toggleReturnForm === "function") toggleReturnForm();
                }, 50);
            }
        }
     });
...
//       $.ajax({    //create an ajax request to load_page.php
//         type:"POST",
//         url: "../backend/cart/controller.php?action=unsetmsg",             
//         dataType: "text",   //expect html to be returned  
