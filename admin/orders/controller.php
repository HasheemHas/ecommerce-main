...
function doInsert(){
	...
	function doEdit(){
		...
		$currentSummary = $existingSummary->single_summary($_GET['id']);
		$originalRemarks = isset($currentSummary->ORDEREDREMARKS) ? $currentSummary->ORDEREDREMARKS : '';
		
		if ($_GET['actions']=='confirm') {
				$status	= 'Confirmed';	
				$remarks ='Your order has been confirmed. ' . (strpos($originalRemarks, 'UTR:') !== false ? $originalRemarks : '');
				$delivered = Date('Y-m-d');

		}elseif ($_GET['actions']=='deliver') {
				$status	= 'Delivered';	
				$remarks ='Your order has been delivered. ' . (strpos($originalRemarks, 'UTR:') !== false ? $originalRemarks : '');
				$delivered = Date('Y-m-d');
				 
		}elseif ($_GET['actions']=='cancel'){
				$status	= 'Cancelled';
				$remarks ='Your order has been cancelled. ' . (strpos($originalRemarks, 'UTR:') !== false ? $originalRemarks : '');
		}
