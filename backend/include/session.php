...
function product_exists($pid){
    $pid=intval($pid);
    $max=(isset($_SESSION['gcCart']) && is_array($_SESSION['gcCart'])) ? count($_SESSION['gcCart']) : 0;
    $flag=0;
    for($i=0;$i<$max;$i++){
...
function product_existstwo($ordernum){
 ...
 if($ordernum<1) return;
    ...
    if(is_array($_SESSION['gcCarttwo'])){
      ...
      $max=(isset($_SESSION['gcCarttwo']) && is_array($_SESSION['gcCarttwo'])) ? count($_SESSION['gcCarttwo']) : 0;
